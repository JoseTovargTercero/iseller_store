  ...
  $MENU_TOGGLE.on("click", function () {
    ...
    if ($BODY.hasClass("nav-md")) {
      ...
      $SIDEBAR_MENU
        ...
        .addClass("active-sm")
        .removeClass("active");
      $(".nav_title").css("text-align", "center");
      $(".nav_title").css("width", "97%");
    } else {
...
function formatPeso(amount) {
  ...
  // Convertir el número a un string con separadores de miles
  let formattedAmount = roundedAmount.toLocaleString("en-US");

  return roundedAmount;
}
function recortarADosDecimales(numero) {
...
function redondearAlSiguienteCien(numero) {
  return Math.ceil(numero / 100) * 100;
}

function toggleView(vista) {
  document.getElementById(vista).classList.toggle("hide");
}
    ...
    return async (...args) => {
      ...
      try {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          console.log("DEBUG fetch (JSON):", json);
        } else {
          const text = await clone.text();
          console.log("DEBUG fetch (texto):", text);
        }
      } catch (err) {
        console.warn("No se pudo leer la respuesta fetch:", err);
      }
...
if (DEBUG) {
  $(document).ajaxSuccess(function (event, xhr, settings) {
    console.log("AJAX éxito:", xhr.responseText);
  });
